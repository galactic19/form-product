  ...
  return (
    <section className="border-t border-border px-6 py-24 sm:py-32" aria-hidden="true">
      <div className="mx-auto max-w-7xl space-y-4">
        ...
        <div className="h-10 w-full max-w-3xl animate-pulse bg-surface-bright" />
        <div className="h-5 w-full max-w-2xl animate-pulse bg-surface-bright" />
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="h-36 animate-pulse border border-border bg-[color:var(--surface-elevated)]"
            />
          ))}
        </div>
      </div>
